...
public class PacketHandler : Attribute
{
    ...
    /// must be the <see cref="IClientConnection"/>.
    /// </summary>
    ...
    internal static void CallHandler(IPacket packet)
    {
    ...
    internal static void CallHandler(IClientConnection client, IPacket packet)
    {
        var type = packet.GetType();
        if (!RegisteredHandlers.ContainsKey(type)) return;
