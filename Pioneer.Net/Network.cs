...
public static class Network
{
    ...
    public static async Task ConnectAsync(string ip, int port, string password, Action<IClientConnection> callback)
    {
        try
        {
            var client = new Client(ip, port, password);
            await client.ConnectServer(callback);
        }
        catch (Exception e)
        {
    ...
    public static async Task<IServer?> CreateAsync(bool isLocal, int port, string? password = null)
    {
        try
        {
            var server = new Server(isLocal, port, password);
